...
def delete_todo(position):
    ...
    with conn:
        cur.execute("DELETE FROM todos WHERE position=%s", (position,))
...
def update_todo(position: int, task: str, category: str):
    with conn:
        if task is not None and category is not None:
            cur.execute('UPDATE todos SET task = %s, category = %s WHERE position = %s',
                      (task, category, position))
        elif task is not None:
            cur.execute('UPDATE todos SET task = %s WHERE position = %s',
                      (task, position))
        elif category is not None:
            cur.execute('UPDATE todos SET category = %s WHERE position = %s',
                      (category, position))
